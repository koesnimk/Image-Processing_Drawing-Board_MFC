...
void IppHoughCircle(IppByteImage& img, std::vector<IppCircleParam>& circles, int threshold)
{
	int w = img.GetWidth();
	int h = img.GetHeight();
	...
	int max_radius = min(w, h) / 2;

	int num_radius = max_radius;
	int num_theta = 360;

	float* sin_tbl = new float[num_theta];
	float* cos_tbl = new float[num_theta];

	for (int i = 0; i < num_theta; i++)
	{
		sin_tbl[i] = sin(i * PI_F / num_theta);
		cos_tbl[i] = cos(i * PI_F / num_theta);
	}

	IppIntImage imgAcc(num_theta, num_radius);
	int** pAcc = imgAcc.GetPixels2D();
		...
		for (int x = 0; x < w; x++)
		{
			if (ptr[y][x] > 128)
			{
				for (int r = 0; r < num_radius; r++)
				{
					...
					{
						int a = static_cast<int>(x - r * cos_tbl[theta]);
						int b = static_cast<int>(y - r * sin_tbl[theta]);
	...
	// 국지적 최대값을 찾아 원 성분으로 결정
	circles.clear();
	for (int r = 0; r < num_radius; r++)
	{
		for (int theta = 0; theta < num_theta; theta++)
		{
			if (pAcc[r][theta] > threshold)
			{
				circles.push_back(IppCircleParam(r, theta, pAcc[r][theta]));
			}
		}
	}
